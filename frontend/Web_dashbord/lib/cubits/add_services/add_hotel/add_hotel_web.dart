import 'dart:convert';

import 'package:web_dashbord/assest/data%20and%20model/add_hotel_model.dart';
import 'package:web_dashbord/screens/log_in.dart';
import 'package:web_dashbord/url_api.dart';
import 'package:http/http.dart' as http;

class AddHotelWeb {
  Future<int?> fetchAddHotel(
    final String? name,
    final String? email,
    final String? phone,
    final String? location,
    final String? priceforNight,
  ) async {
    try {
      print('trying web');
      String body = jsonEncode(AddHotelModel(
        name,
        email,
        phone,
        location,
        priceforNight,
      ).toJson());
      final response = await http.post(Uri.parse('$baseUrl/addHotel'),
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer $token',
          },
          body: body);
      print('$body');

      if (response.statusCode == 200) {
        print(response.body);
        final bookingResponse = HResponse1.fromJson(json.decode(response.body));
        print('${bookingResponse.id}');
        hId1.setId(bookingResponse.id);
        print(bookingResponse);
        print('${response.statusCode}');
        return bookingResponse.id;
      } else {
        print('${response.statusCode}');
        print(response.body);
      }
    } catch (e) {
      print('Exception: $e');
    }
    return null;
  }
}

class HResponse1 {
  final int id;
  HResponse1({required this.id});

  factory HResponse1.fromJson(Map<String, dynamic> json) {
    return HResponse1(id: json['hotel']['id']);
  }
}

class HId1 {
  int? _addId;

  void setId(int addId) {
    _addId = addId;
  }

  int? getId() {
    return _addId;
  }
}

final hId1 = HId1();

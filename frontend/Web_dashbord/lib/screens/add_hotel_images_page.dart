...
class _AddHotelImagesPageState extends State<AddHotelImagesPage> {
  ...
  Future uploadImage1() async {
    String TripId1 = hId1.getId().toString();
    var url = Uri.parse('$baseUrl/photoHotel/$TripId1');
  ...
  String TripId2 = hId2.getId().toString();

  Future uploadImage2() async {
    var url = Uri.parse('$baseUrl/manyPhotoRoom/$TripId2');
    var request = http.MultipartRequest("Post", url);
    request.files.add(await http.MultipartFile.fromBytes(
        'image', selectedFile2!,
        contentType: MediaType('image', '*'), filename: "any name.png"));
